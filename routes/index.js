...
router.post('/UpFile',upload.single('avatar'),function (rep,res,){
 const file = rep.file;
    ...
    if (!file){
      ...
      res.render('Upload',{message:'Thành Công!!!'})
...
router.get('/Uploadfile',function (rep,res) {
  res.render('Upload',{title:'',message:''});
});

////////////////////////////////////
var db = 'mongodb+srv://HuuPhuc:NHUUphuc672002@cluster0.ns7fk.mongodb.net/VewAnh?retryWrites=true&w=majority'
...
router.get('/hotdownloaded',function (rep,res) {
  res.render('Hotdownloaded',{title:'Hot Downloaded'});
});
router.get('/Sunset',function (rep,res) {
  res.render('Sunset',{title:'Sunset'});
});
router.get('/Digital',function (rep,res) {
  res.render('Digital',{title:'Digital'});
});
router.get('/Landscape',function (rep,res) {
  res.render('Landscape',{title:'Landscape'});
});
router.get('/Anime',function (rep,res) {
  res.render('Anime',{title:'Anime'});
});
router.get('/introduce',function (rep,res) {
  res.render('Introduce',{title:'Đăng Ký Tài Khoản',message:''});
});

router.get('/suaAnh',function (rep,res) {
  res.render('SuaAnh',{title:'',message:''});
});
router.get('/themAnh',function (rep,res) {
  res.render('ThemAnh',{title:'',message:''});
});

// code chuyền dữa liệu//////////////////////////////////////////////////////////////////////////////////////
...
var VewAnh=mongoose.model('VewAnh',VewAnhs)
router.post('/ThemHAnh',function (repuest,response){
  var ntenAnh=repuest.body.TenAnh;
  var nndAnh=repuest.body.NDAnh;
  var nlinkAnh=repuest.body.LinkAnh;

  console.log(ntenAnh+nndAnh+nlinkAnh);

  const data = new VewAnh( {
    TenAnh:ntenAnh,
    NDAnh:nndAnh,
    LinkAnh:nlinkAnh
  });
...
router.post('/SuaHAnh',function (repuest,response){
  ...
  console.log(ntenAnh+nndAnh+nlinkAnh);
  VewAnh.updateOne({TenAnh:ntenAnh},{TenAnh:ntenAnh,NDAnh:nndAnh,LinkAnh:nlinkAnh},function (err) {
    if (err) throw err;

  })

})
router.post('/xaoAnh',function (repuest,response){
  var ntenAnh=repuest.body.TenAnh;


  console.log(ntenAnh);
  VewAnh.deleteOne({TenAnh:ntenAnh},function (err) {
    if (err) throw err;
...
//   },]
//   res.render('View',{title:'',VewAnh:VewAnh});
  VewAnh.find({},function (err,data) {
    res.render('View',{data:data})

  })
});

router.post('/DangKy',function (repuest,response){
  var nEmail  =repuest.body.Email;
  var nPhone  =repuest.body.Phone;
  var nContet =repuest.body.Content;

  fs.writeFile('Uploads/' + 'uploads.txt',  nEmail+' \n'+nPhone+'\n'+   nContet,function (error) {
    if (error){
      response.render('Introduce',{message:error})

    }else {
      response.render('Introduce',{message:'Bạn Đã Đăng Ký Thành Công !'})
    }
  })
